...
// Enhanced Camera Component for both mobile and desktop
const WebCameraView = ({ onCapture, onCancel }) => {
  ...
  const initializeCamera = async () => {
    try {
      ...
      // Request camera permission - works on desktop laptops too
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { 
          facingMode: { ideal: 'environment' }, // Prefer back camera, falls back to front
          width: { ideal: 1280 },
          height: { ideal: 720 }
        }
      });
            ...
            <View style={styles.detailsContainer}>
              {results.market_insights && (
                <View style={styles.insightSection}>
                  ...
                  <Text style={styles.insightText}>{results.market_insights}</Text>
                </View>
              )}

              {results.selling_tips && (
                <View style={styles.insightSection}>
                  <Text style={styles.insightTitle}>💡 Selling Tips</Text>
                  <Text style={styles.insightText}>{results.selling_tips}</Text>
                </View>
              )}

              {results.brand_context && (
                <View style={styles.insightSection}>
                  <Text style={styles.insightTitle}>🏷️ Brand Context</Text>
                  <Text style={styles.insightText}>{results.brand_context}</Text>
                </View>
              )}

              {results.seasonal_notes && (
                <View style={styles.insightSection}>
                  <Text style={styles.insightTitle}>🌤️ Seasonal Notes</Text>
                  <Text style={styles.insightText}>{results.seasonal_notes}</Text>
                </View>
              )}
            </View>
          )}
